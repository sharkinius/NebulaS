...
function renderAccounts() {
  ...
  state.accounts.forEach((acc, idx) => {
    ...
    kebab.textContent = '⋮';
    kebab.onclick = (ev) => { ev.stopPropagation(); toggleMenu(li, idx); };
...
function toggleMenu(container, idx) {
  closeMenus();
  const menu = document.createElement('div');
  menu.className = 'menu';
  const del = document.createElement('button'); del.textContent = 'Удалить аккаунт';
  del.onclick = (e) => { e.stopPropagation(); confirmDelete(idx); };
...
function confirmDelete(idx) {
  ...
  if (!acc) return;
  if (!confirm(`Удалить аккаунт ${acc.user.username}?`)) return;
  state.accounts.splice(idx, 1);
  if (state.activeIndex === idx) { state.activeIndex = -1; state.token = null; state.user = null; }
  if (state.activeIndex > idx) state.activeIndex -= 1;
  saveAccounts();
  renderAccounts();
  updateAuthGate();
