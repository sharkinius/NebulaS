// Avoid double-load on GitHub Pages/extensions
window.API_BASE = window.API_BASE || 'https://nebula-bmyh.onrender.com';

window.__nebulaState = window.__nebulaState || {
...
const state = window.__nebulaState;

// Prevent any form default navigation on GitHub Pages
document.addEventListener('submit', (e) => { e.preventDefault(); return false; });

// Attach explicit click handlers to buttons to avoid default form submission
document.addEventListener('DOMContentLoaded', () => {
  const regBtn = document.getElementById('reg-submit');
  const loginBtn = document.getElementById('login-submit');
  if (regBtn && !regBtn._bound) {
    regBtn._bound = true;
    regBtn.addEventListener('click', async (e) => {
      ...
      try {
        ...
        toast('Зарегистрирован: ' + data.user.username);
  ...
  if (loginBtn && !loginBtn._bound) {
    ...
    loginBtn.addEventListener('click', async (e) => {
      ...
      try { const data = await api('/api/login', { method: 'POST', body: { username, password } }); addAccount(data.user, data.token); updateAuthGate(); } catch { toast('Неверные данные'); }
    });
  }
});
...
function connectSocket() {
  ...
  state.socket.on('message:new', (msg) => {
    if ((msg.fromUserId === state.activeFriend?.id && msg.toUserId === state.user?.id) || (msg.toUserId === state.activeFriend?.id && msg.fromUserId === state.user?.id)) {
      state.messages.push(msg); renderMessages();
    }
  });
  ...
  state.socket.on('group:message', (msg) => {
    if (state.activeGroup && msg.groupId === state.activeGroup.id) { state.groupMessages.push(msg); renderGroupMessages(); }
  });
}
...
document.getElementById('register-form').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    toast(msg.includes('Ник занят') || msg.includes('username taken') ? 'Ник занят' : msg);
  }
  updateAuthGate();
});
...
document.getElementById('login-form').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    addAccount(data.user, data.token);
  } catch (err) { toast('Неверные данные'); }
  updateAuthGate();
});
...
function renderAccounts() {
  ...
  state.accounts.forEach((acc, idx) => {
    ...
    if (isOwner(acc.user.username)) { const badge = document.createElement('span'); badge.className = 'badge-owner'; badge.textContent = 'OWNER'; li.appendChild(badge); }
...
searchInput.addEventListener('input', async () => {
  ...
  ul.innerHTML = '';
  results.filter(u => u.id !== state.user?.id).forEach(u => {
    ...
    btn.onclick = async () => { await api('/api/friends/request', { method: 'POST', body: { toUserId: u.id } }); toast('Заявка отправлена'); };
...
async function loadFriends() {
  ...
  friends.forEach(f => {
    const li = document.createElement('li'); li.className = 'friend';
    const name = document.createElement('span'); name.textContent = f.username;
    if (isOwner(f.username)) { const badge = document.createElement('span'); badge.className = 'badge-owner'; badge.textContent = 'OWNER'; li.appendChild(badge); }
    const btn = document.createElement('button'); btn.textContent = 'Открыть чат'; btn.onclick = () => openChat(f);
...
async function loadRequests() {
  ...
  reqs.forEach(r => {
    const li = document.createElement('li'); li.className = 'request-item friend';
    const name = document.createElement('span'); name.textContent = `${r.fromUsername}`;
    const btn = document.createElement('button'); btn.textContent = 'Принять';
    btn.onclick = async () => { await api('/api/friends/accept', { method: 'POST', body: { fromUserId: r.fromUserId } }); loadFriends(); loadRequests(); };
    li.appendChild(name); li.appendChild(btn); ul.appendChild(li);
  });
}
...
async function openChat(friend) {
  ...
  renderMessages(); showSections({ chat: true });
}

function renderMessages() {
  const ul = document.getElementById('messages'); ul.innerHTML = '';
  state.messages.forEach(m => {
    const li = document.createElement('li'); li.className = 'message' + (m.fromUserId === state.user?.id ? ' me' : '');
...
document.getElementById('message-form').addEventListener('submit', async (e) => {
  e.preventDefault(); if (!state.activeFriend) return;
  const input = document.getElementById('message-input'); const content = input.value.trim(); if (!content) return;
  try { await api('/api/messages/send', { method: 'POST', body: { toUserId: state.activeFriend.id, content } }); } catch (err) { toast('Не удалось отправить: ' + err.message); }
  input.value=''; renderMessages();
});

// Initial
renderAccounts(); if (state.activeIndex >= 0 && state.accounts[state.activeIndex]) setActiveAccount(state.activeIndex);
...
function showSections({ chat = false, groups = false } = {}) {
  if (chat) document.getElementById('chat-section').hidden = false;
  if (groups) document.getElementById('groups-section').hidden = false;
}

document.getElementById('create-group').onclick = async () => {
  if (!state.token) return toast('Сначала войдите');
  ...
  await loadGroups(); openGroup({ id: data.id, name: data.name });
};

async function pickMembers() {
  const friends = await api('/api/friends'); if (friends.length === 0) return [];
  const names = friends.map(f => f.username).join(', ');
  const chosen = prompt('Участники (через запятую), из: ' + names); if (!chosen) return [];
  const set = new Set(chosen.split(',').map(s => s.trim()).filter(Boolean));
  return friends.filter(f => set.has(f.username)).map(f => f.id);
}
...
async function loadGroups() {
  ...
  groups.forEach(g => { const li = document.createElement('li'); li.className = 'friend'; const name = document.createElement('span'); name.textContent = g.name; const btn = document.createElement('button'); btn.textContent = 'Открыть'; btn.onclick = () => openGroup(g); li.appendChild(name); li.appendChild(btn); ul.appendChild(li); });
}

async function openGroup(group) {
  state.activeGroup = group; state.groupMessages = await api('/api/groups/history/' + group.id);
  document.getElementById('chat-title').textContent = 'Группа: ' + group.name; renderGroupMessages(); showSections({ groups: true });
}

function renderGroupMessages() {
  const ul = document.getElementById('messages'); ul.innerHTML = '';
  state.groupMessages.forEach(m => { const li = document.createElement('li'); li.className = 'message' + (m.fromUserId === state.user?.id ? ' me' : ''); const meta = document.createElement('div'); meta.className = 'meta'; const date = new Date(m.createdAt); const ownerTag = (isOwnerId(m.fromUserId) ? '<span class="badge-owner-inline">OWNER</span> ' : ''); meta.innerHTML = ownerTag + 'Участник • ' + date.toLocaleTimeString(); const body = document.createElement('div'); body.textContent = m.content; li.appendChild(meta); li.appendChild(body); ul.appendChild(li); }); ul.scrollTop = ul.scrollHeight;
}

function isOwner(username) { return username === 'Nebula' || username === 'NebulaTest'; }
function isOwnerId(userId) { const acc = state.accounts.find(a => a.user.id === userId); if (acc) return isOwner(acc.user.username); return false; }

function setupMobile(authed) {
  const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
  const bar = document.getElementById('mobile-bar'); const sidebar = document.getElementById('sidebar');
  if (!authed) { bar.hidden = true; sidebar.classList.remove('open'); return; }
  bar.hidden = !isMobile; const btn = document.getElementById('toggle-sidebar');
  if (btn && !btn._bound) { btn._bound = true; btn.addEventListener('click', () => { sidebar.classList.toggle('open'); }); }
  document.addEventListener('click', (e) => { if (!isMobile) return; if (!sidebar.classList.contains('open')) return; if (!sidebar.contains(e.target) && e.target !== btn) sidebar.classList.remove('open'); });
}

function toast(text) { const t = document.getElementById('toast'); t.textContent = text; t.hidden = false; clearTimeout(t._to); t._to = setTimeout(()=>{ t.hidden = true; }, 2000); }
